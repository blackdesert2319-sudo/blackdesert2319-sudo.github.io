...
function generateMultiSelectMultiGroupCount(payload, database) {
    ...
    for (let i = 0; i < numGroups; i++) {
        ...
        do {
            ...
            attempts++;
        } while (wrongCount === counts[i] && attempts < 10); // Đảm bảo số sai khác số đúng
    ...
    // Lấy 2 câu đúng (hoặc 1 nếu chỉ có 1)
    options.push(correctStmts.pop());
    if (correctStmts.length > 0) {
        options.push(correctStmts.pop());
    } else {
        options.push(wrongStmts.pop()); // Lấy thêm 1 câu sai nếu không đủ 2 câu đúng
    }
    
    // Lấy 2 câu sai
    options.push(wrongStmts.pop());
    if (wrongStmts.length > 0) {
         options.push(wrongStmts.pop());
    } else {
         options.push(correctStmts.pop()); // Lấy thêm 1 câu đúng nếu k đủ 2 câu sai
    }
...
function setupSubmitButton(correctAnswer) {
    ...
    newButton.addEventListener('click', () => {
        ...
        multiSelectButtons.forEach(button => {
            ...
            if (correctAnswer.hasOwnProperty(choiceId)) {
